        ...
        <div class="w-full p-6 py-12 mx-auto space-y-24 lg:px-8 lg:max-w-7xl">
            <div class="relative block">
                @if(isset($movie['backdrop_path']))
                    <img src="https://image.tmdb.org/t/p/original/{{$movie['backdrop_path']}}" alt="{{$movie['title']}}" class="rounded-lg rounded-b-none opacity-25">
                @endif
                <div class="lg:absolute top-0 w-full block grid-cols-1 justify-content-center justify-content-middle mt-4">
                    <h2 class="text-3xl font-bold tracking-tight text-center sm:text-5xl text-gray-50">{{$movie['title']}}</h2>
                    <p class="max-w-3xl mx-auto mt-4 text-xl text-center text-gray-400">{{$movie['overview']}}</p>
                </div>
            </div>
            @if (isset($movie['watch/providers']['results'][Cookie::get('locale') ]))
                @if (isset($movie['watch/providers']['results'][Cookie::get('locale') ]['flatrate']))
                <div>
                    <h2 class="text-3xl font-thin uppercase tracking-tight text-center sm:text-5xl text-gray-50">Subscription streams</h2>
                        ...
                        @foreach($movie['watch/providers']['results'][Cookie::get('locale') ]['flatrate'] as $provider) 
                ...
                @endif
            @endif
            @if (isset($movie['providers']['results'][Cookie::get('locale') ]))
                @if (isset($movie['providers']['results'][Cookie::get('locale') ]['flatrate']))
                    <div>
                        <h2 class="text-3xl font-thin uppercase tracking-tight text-center sm:text-5xl text-gray-50">Subscription streams</h2>                        
                            <div class="grid lg:grid-cols-3 grid-cols-1 mt-2 gap-4">
                            @foreach($movie['providers']['results'][Cookie::get('locale') ]['flatrate'] as $provider) 
                                <div class="flex overflow-hidden rounded-lg bg-gray-900 text-gray-100">
                                    <div class="flex items-center justify-center px-4 bg-violet-400 text-gray-800">
                                        <img src="https://image.tmdb.org/t/p/original/{{$provider['logo_path']}}" alt="{{ $provider['provider_name'] }}" class="h-8 rounded mx-4">
                                    </div>
            ...
            @if (isset($movie['watch/providers']['results'][Cookie::get('locale') ]))
                @if (isset($movie['watch/providers']['results'][Cookie::get('locale') ]['rent']))
                <div>
                    <h2 class="text-3xl font-thin uppercase tracking-tight text-center sm:text-5xl text-gray-50">Rentable</h2>
                    
                        <div class="grid lg:grid-cols-3 grid-cols-1 mt-2 gap-4">
                        @foreach($movie['watch/providers']['results'][Cookie::get('locale') ]['rent'] as $provider) 
                            <div class="flex overflow-hidden rounded-lg bg-gray-900 text-gray-100">
            ...
            @endif
            @if (isset($movie['providers']['results'][Cookie::get('locale') ]))
                @if (isset($movie['providers']['results'][Cookie::get('locale') ]['rent']))
                    <div>
                        <h2 class="text-3xl font-thin uppercase tracking-tight text-center sm:text-5xl text-gray-50">Rentable</h2>                        
                            <div class="grid lg:grid-cols-3 grid-cols-1 mt-2 gap-4">
                            @foreach($movie['providers']['results'][Cookie::get('locale') ]['rent'] as $provider) 
                                <div class="flex overflow-hidden rounded-lg bg-gray-900 text-gray-100">
                                    <div class="flex items-center justify-center px-4 bg-blue-400 text-gray-800">
                                        <img src="https://image.tmdb.org/t/p/original/{{$provider['logo_path']}}" alt="{{ $provider['provider_name'] }}" class="h-8 rounded mx-4">
                                    </div>
            ...
            @if (isset($movie['watch/providers']['results'][Cookie::get('locale') ]))
                @if (isset($movie['watch/providers']['results'][Cookie::get('locale') ]['buy']))
                <div>
                    <h2 class="text-3xl font-thin uppercase tracking-tight text-center sm:text-5xl text-gray-50">Available to rent</h2>
                    
                        <div class="grid lg:grid-cols-3 grid-cols-1 mt-2 gap-4">
                        @foreach($movie['watch/providers']['results'][Cookie::get('locale') ]['buy'] as $provider) 
                            <div class="flex overflow-hidden rounded-lg bg-gray-900 text-gray-100">
            ...
            @endif
            @if (isset($movie['providers']['results'][Cookie::get('locale') ]))
                @if (isset($movie['providers']['results'][Cookie::get('locale') ]['buy']))
                    <div>
                        <h2 class="text-3xl font-thin uppercase tracking-tight text-center sm:text-5xl text-gray-50">Purchasable</h2>                        
                            <div class="grid lg:grid-cols-3 grid-cols-1 mt-2 gap-4">
                            @foreach($movie['providers']['results'][Cookie::get('locale') ]['buy'] as $provider) 
                                <div class="flex overflow-hidden rounded-lg bg-gray-900 text-gray-100">
                                    <div class="flex items-center justify-center px-4 bg-green-400 text-gray-800">
                                        <img src="https://image.tmdb.org/t/p/original/{{$provider['logo_path']}}" alt="{{ $provider['provider_name'] }}" class="h-8 rounded mx-4">
                                    </div>
            ...
            <div class="grid lg:gap-8 lg:grid-cols-2 lg:items-center">
                <div>
                    <div class="mt-12 space-y-12">
                        ...
                        @if (isset($movie['genres']))
                            <div class="flex">
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="mt-2 text-gray-400 grid sm:grid-cols-2 grid-cols-3 gap-2">
                        ...
                        @endif
                        @if (count($movie['production_companies']))
                            <div class="flex">
                                <div class="flex-shrink-0">
                ...
                <div aria-hidden="true" class="mt-10 lg:mt-0 relative">
                    @if($movie['poster_path'])
                        <img src="https://image.tmdb.org/t/p/original/{{$movie['poster_path']}}" alt="" class="mx-auto rounded-lg shadow-lg dark:bg-coolGray-500">
                    @endif
                    <span class="absolute top-0 flex">
                        <span class="text-sm font-bold mr-2 px-2.5 py-0.5 rounded bg-purple-900 text-purple-200 uppercase">{{ $movie['status'] }}: {{\Carbon\Carbon::parse($movie['release_date'])->format('d/m/y')}}</span>
                    </span>
                </div>
            </div>
